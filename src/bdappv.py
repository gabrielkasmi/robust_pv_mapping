...
class BDAPPVClassification(Dataset):
    def __init__(self, img_dir, transform = None, size = 299, random = True, images_list = None, noise = False, downsample = None):
        """
        Args:
        ...
        self.random = random # randomly crop the image. If none, cropped from the center.
        ...
        if images_list is None:
            ...
            # images list defines a list of images from the img folder. to select the masks (i.e. positive labels)
            # corresponding to these iamges, we intersec the cmplete list of masks with the list of images 
            # passed as input. 

            self.img_folder = [m for m in os.listdir(self.img_dir) if (m[-4:]=='.png') & (m in images_list)]
            self.mask_folder = [m for m in os.listdir(self.mask_dir) if( m[-4:] == '.png') & (m in images_list)]       
    ...
    def __getitem__(self, idx):
        ...
        if self.noise:
            ...
            img = random_noise(np.array(img), mode='gaussian', mean=0, var=0.1, clip = True, seed = 42)
            img = Image.fromarray((NormalizeData(img) * 255).astype(np.uint8))

        if self.downsample is not None:

            img = img.resize((self.downsample, self.downsample))
    ...
    def crop(self, image, label):
        ...
        if label == 0:
            ...
            image, x, y = apply_crop(image, self.size, self.random)
...
class BDAPPVClassificationManual(Dataset):
    def __init__(self, img_dir, masks_dir, transform = None, size = 299, random = True, opt = None, specialized = False):
        """
        Args:
            img_dir (str): directory with all the images. Should have the following structure: 
