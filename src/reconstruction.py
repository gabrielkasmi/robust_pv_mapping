...
def compute_sparse_masks(arr):
    ...
    masks = np.insert(masks, 0, np.zeros((arr.shape[0], arr.shape[1])).astype(np.uint8), axis = 2)
    return masks


def reconstruct_images_from_single_mask(image, mask, levels = 3):
    """
    ...
    return inverse_wavelet(wt, upsampled_mask)
...
def evaluate_on_samples(images, label, model, batch_size = 128, transform = None):
    ...
    with torch.no_grad():
        ...
        for batch_index in range(np_batch):
            ...
            # batch preds
            batch_y = F.softmax(model(batch_x), dim = 1)
